                ...
                <table className="min-w-full divide-y divide-default-200">
                    <thead>
                        <tr>
                            ...
                            <th className="px-3 py-3 text-left text-xs font-medium text-default-500 uppercase tracking-wider">Trx</th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y divide-default-200">
                        {products.map((product, index) => (
                            <tr key={index} className="hover:bg-default-50">
                                <td className="px-3 py-4 whitespace-nowrap text-sm font-medium text-default-900">{product.product_name}</td>
                                <td className="px-3 py-4 whitespace-nowrap text-sm text-default-500">{product.total_moved}</td>
                                <td className="px-3 py-4 whitespace-nowrap text-sm text-success-600">+{product.total_moved_inbound}</td>
                                <td className="px-3 py-4 whitespace-nowrap text-sm text-danger-600">-{product.total_moved_outbound}</td>
                                <td className="px-3 py-4 whitespace-nowrap text-sm text-default-500">{product.transaction_count}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
